...
sequenceDiagram
    ...
    activate server
    Note right of browser:The Browser sends a POST request with the Payload
    server-->>browser: https://studies.cs.helsinki.fi/exampleapp/notes
    deactivate server
    Note left of server: The Server accepts the Payload and responds with URL Redirect
